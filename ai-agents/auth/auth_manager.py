...
from asgardeo.auth.models import ClientConfig, ServerConfig, OAuthToken
...
class AutogenAuthManager:
    ...
    async def get_oauth_token(self, config: AuthConfig) -> Optional[OAuthToken]:
        ...
        # Handle token refresh for expired OBO tokens
        if token and token.is_expired() and config.token_type == OAuthTokenType.OBO_TOKEN:
            logger.debug("Token expired. Attempting to refresh %s for scopes %s", 
                        config.token_type.name, config.scopes)
            try:
                token = await self.agent_auth_manager.refresh_token(token.refresh_token, config.scopes)
                if token:
    ...
    async def _fetch_oauth_token(
        ...
        Args:
            ...
            code_verifier: PKCE code verifier (required for OBO tokens)
        ...
        try:
            if config.token_type == OAuthTokenType.OBO_TOKEN:
                ...
                return await self.agent_auth_manager.get_obo_token_with_pkce(
                    auth_code=code,
